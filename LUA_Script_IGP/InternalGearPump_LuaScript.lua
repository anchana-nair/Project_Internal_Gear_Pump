...
-------------------------------------------------------Internal Gear Pump--------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------------
-----------------------------------Authors: Anchana Radhakrishnan Nair & Dinesh Thirumurugan--------------------------------------
...
---------------------------------------------------------------------------------------------------------------------------------
--COMMENT LINE 506 (Front Flange) TO VIEW THE INNER COMPONENTS OF INTERNAL GEAR PUMP
--DOWNLOAD AND ADD THE font.ttf FILE TO THE WORKING DIRECTORY
...
alpha_t=ui_scalarBox("Pressure Angle(Deg)",20,1);				                                                            -- Pressure angle
width=ui_numberBox("Width(mm)",15);								                                                            -- Width/Thickness of the gear
x_coef_int=ui_scalarBox("Internal Profile Shift(mm)",0,0.1);	                                                            -- Profile shift factor for internal gear
...
rotation = ui_numberBox("Bi-directional Flow",0)*10;                                                               			-- For fluid rotation im both direction
transform_pick = ui_pick('Drag the top flange to move it')																	-- For moving the front flange
...
function internal_gear()
	diff_coeff = x_coef_int - x_coef_ext;
	if diff_coeff < -0.1 then
...
r3=translate(rotation,0,0)																								-- Rotation for fluid
---------------------------------------------------------------------------------------------------------------------------------
--------------------------------------------------------Emitting Section---------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------------
...
internalGear_full=r1*difference(outer_circle,internalGear)
...
externalGear_full=translate(0,meshDistance,0)*r2*difference(flangeFull,flangeDiff)
...
crescentFull_fluid=crescent_Full
...
-----------------------------------------------------Piping-----------------------------------------------------------------------
pipeForm1=rotate(270,X)*difference(cylinder(width/2+2,internal_radius*2), translate(0,-(width/2)+2,0)*cylinder(width/2-1,internal_radius*2))	
emit(rotate(45,Z)*translate(0,internal_radius+2+31.5,3)*pipeForm1,22)														-- Piping left-side
pipeForm2=rotate(270,-Y)*difference(cylinder(width/2+2,internal_radius*2), translate(-(width/2)+2,0,0)*cylinder(width/2-1,internal_radius*2))
emit(rotate(45,Z)*translate(internal_radius+2+31.5,0,3)*pipeForm2,22)														-- Piping right-side
---------------------------------------------------Fluid  flow------------------------------------------------------------
cube_fluidCircle=extrude(circle(internal_radius+6), 0, v(0,0,width), v(1,1,1), 20)	
cube_diffFluid=difference(difference(cube_fluidCircle,internalGear_full),externalGear_full)
cube_fluid=difference(difference(cube_diffFluid,fln),crescentFull_fluid)													-- Cube formation for fluid flow

cuble_fluidIntgear=difference(cylinder(internal_radius,width),cylinder(crescent_rootRadius,width))
fluid_diff=difference(cuble_fluidIntgear,internalGear_full)
fluid_diffTotal=difference(translate(0,meshDistance,0)*cylinder(crescent_innerRadius,width),externalGear_full)				-- Difference for fluid flow


if(rotation~=0) then

	if(rotation>0) then
			fluid_cubeForm=translate(-internal_radius*2+2,0,0)*r3*cube(internal_radius*2+10,2*internal_radius+32, width)
			fluid_cubeDiff =(difference(fluid_cubeForm,cylinder(internal_radius+2,width-2)))
			fluid_cubeDifftotal= difference(fluid_cubeForm,fluid_cubeDiff)
			fluid_cc=intersection(fluid_cubeDifftotal,cube_fluid)
			emit(fluid_cc,10)																										-- Fluid flow direction
			...
			emit(rotate(46,Z)*translate(1,internal_radius+2,width/2-1)*inner_fluid2,10)											-- Left-side fluid inner flow fluid
		
		if(rotation>internal_radius*2 and rotation<=(internal_radius+2)*4.5) then
			...
			emit(rotate(46,Z)*translate(internal_radius+2,-0.5,width/2-1)*inner_fluid4,10)										-- Right-side fluid outer flow
	...
	elseif(rotation<0) then
			...
			fluid_cubeForm1=translate(internal_radius*2+2,0,0)*r3*cube(internal_radius*2+10,2*internal_radius+32, width)
			fluid_cubeDiff1 =(difference(fluid_cubeForm1,cylinder(internal_radius+2,width-2)))
			fluid_cubeDifftotal= (difference(fluid_cubeForm1,fluid_cubeDiff1))
			fluid_c=intersection(fluid_cubeDifftotal,cube_fluid)
			emit(fluid_c,10)																									-- Fluid flow direction
			...
			emit(rotate(46,Z)*translate(internal_radius+2,-0.5,width/2-1)*inner_fluid_c1,10)									-- Right-side fluid inner flow
	
		if(rotation<-(internal_radius-2)*2 and rotation>=-(internal_radius+2)*4.5) then
			...
			emit(rotate(46,Z)*translate(1,internal_radius+2,width/2-1)*outer_fluid2,10)											-- Left-side fluid outer flow			
		end
...
emit(transform_pick*intersection(output_upperCase1,output_upperCase),50)													 -- Front flange with screw
...
emit(translate(0,0,-10)*cube(3*radius+addOn_distance,3*radius+addOn_distance,0.1),60)                              			-- Rectangular Base Plate Formation
...
set_brush_color(10,0.300,0.600,0.900)																						-- Fluid color
set_brush_color(15,0.60,0.947,0.942)
set_brush_color(8,0.315,0.535,0.963)
---------------------------------------------------Fluid Colour-----------------------------------------------------------------
set_brush_color(21,0.455,0.599,0.838)
set_brush_color(22,0.422,0.449,0.493)																						-- Casing and port color
...
output = translate(6,-1.5*radius+addOn_distance-8,-10)*scale(0.3,0.3,1)*f_n:str('Optimal Gear Solutions',3)             -- Font Location and Definition
emit(output)
